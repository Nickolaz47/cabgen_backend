...
func TestAssignTicket(t *testing.T) {
	...
	t.Run("Error - Ticket Is Not Open", func(t *testing.T) {
		svc := &mocks.MockTicketService{
			AssignFunc: func(ctx context.Context, ticketID, adminID uuid.UUID) (
				*models.TicketResponse, error) {
				return nil, services.ErrTicketIsNotOpen
			},
		}
		handler := ticket.NewAdminTicketHandler(svc)
		...
		expected := testutils.ToJSON(
			map[string]string{
				"error": "Cannot assign a ticket that is in progress or already resolved to an admin.",
			},
		)
