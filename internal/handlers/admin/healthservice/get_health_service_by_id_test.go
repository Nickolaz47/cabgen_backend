...
func TestGetHealthServiceByID(t *testing.T) {
	...
	t.Run("Success", func(t *testing.T) {
		svc := &mocks.MockHealthServiceService{
			FindByIDFunc: func(ctx context.Context, ID uuid.UUID) (
				*models.HealthServiceAdminTableResponse, error) {
				response := mockHealthService.ToAdminTableResponse()
				return &response, nil
			},
		}
		...
		handler.GetHealthServiceByID(c)

		expected := testutils.ToJSON(map[string]any{
			"data": mockHealthService.ToAdminTableResponse(),
		})
	...
	t.Run("Error - Invalid ID", func(t *testing.T) {
		...
		c, w := testutils.SetupGinContext(
			...
			"/api/admin/health-service",
			"", nil, gin.Params{
				{Key: "healthServiceId", Value: "1234"},
			},
		)
		handler.GetHealthServiceByID(c)
