...
func (h *EmailTaskHandler) ProcessTask(ctx context.Context,
	t *asynq.Task) error {
	...
	case tasks.TaskTypeAdminAlertEmail:
		...
		h.logTaskStart(ctx, t, zap.String("new_user_id", p.NewUserID.String()))
		return h.execute(ctx, t, h.EmailService.SendAdminAlertEmail(ctx,
			p.NewUserID))
	...
	case tasks.TaskTypeWelcomeEmail:
		...
		h.logTaskStart(ctx, t, zap.String("user_id", p.UserID.String()))
		return h.execute(ctx, t, h.EmailService.SendWelcomeEmail(ctx, p.UserID))
	...
	case tasks.TaskTypeAnalysisDoneEmail:
		...
		h.logTaskStart(ctx, t, zap.String("analysis_id", p.AnalysisID.String()))
		return h.execute(ctx, t, h.EmailService.SendAnalysisDoneEmail(ctx,
			p.AnalysisID))
	...
	case tasks.TaskTypeAdminTicketEmail:
		...
		h.logTaskStart(ctx, t, zap.String("ticket_id", p.TicketID.String()))
		return h.execute(ctx, t, h.EmailService.SendAdminTicketEmail(ctx,
			p.TicketID))
	...
	case tasks.TaskTypeFinishedTicketEmail:
		...
		h.logTaskStart(ctx, t, zap.String("ticket_id", p.TicketID.String()))
		return h.execute(ctx, t, h.EmailService.SendFinishedTicketEmail(ctx,
			p.TicketID))
	...
	case tasks.TaskTypePasswordResetEmail:
		...
		h.logTaskStart(ctx, t, zap.String("email", p.Email))
		return h.execute(ctx, t, h.EmailService.SendPasswordResetEmail(ctx,
			p.Email, p.Name, p.Token))
	...
	case tasks.TaskTypeUserDeletedEmail:
		...
		h.logTaskStart(ctx, t, zap.String("email", p.Email))
		return h.execute(ctx, t, h.EmailService.SendUserDeletedEmail(ctx,
			p.Email, p.Name))
	...
	case tasks.TaskTypeEmailUpdateConfirmation:
		...
		return h.execute(ctx, t, h.EmailService.SendEmailUpdateConfirmation(
			ctx, p.Email, p.Name, p.OldEmail, p.NewEmail, p.Token))
...
func (h *EmailTaskHandler) logTaskStart(ctx context.Context,
	t *asynq.Task, fields ...zap.Field) {
...
func (h *EmailTaskHandler) execute(ctx context.Context, t *asynq.Task,
	err error) error {
	if err != nil {
